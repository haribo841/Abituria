...
> [!IMPORTANT]
> Opublikowane paczki należy pobierać wyłącznie ze zweryfikowanego GitHub Release. Bieżący worktree ma `releaseEligible=true`, a rozszerzenie deklaracji praw z 1 września 2026 r. obejmuje wszystkie paczkowane arkusze CKE i diagramy. Każdą następną paczkę nadal poprzedza pełna walidacja proweniencji.
...
Po zatwierdzeniu wydania pobierz z [GitHub Release `v0.9.3`](https://github.com/haribo841/Abituria/releases/tag/v0.9.3) dokładnie jeden artefakt uruchomieniowy:

- `Abituria-v0.9.3-win-x64.exe` dla Windows;
- `Abituria-v0.9.3-linux-x64.tar.gz`;
- `Abituria-v0.9.3-osx-x64.zip`.

Pobierz również `SHA256SUMS.txt`. Dla Windows archiwum `Abituria-v0.9.3-win-x64.zip` pozostaje opcjonalnym pakietem audytowym z licencjami, notices i SBOM. Każdemu artefaktowi uruchomieniowemu odpowiada SBOM SPDX oraz dwie atestacje wygenerowane przez workflow wydania: domyślna SLSA build provenance i atestacja dokumentu SBOM.
...
```powershell
$app = ".\Abituria-v0.9.3-win-x64.exe"
...
```bash
sha256sum Abituria-v0.9.3-linux-x64.tar.gz
grep 'Abituria-v0.9.3-linux-x64.tar.gz' SHA256SUMS.txt
gh attestation verify Abituria-v0.9.3-linux-x64.tar.gz --repo haribo841/Abituria
```

### macOS

```bash
shasum -a 256 Abituria-v0.9.3-osx-x64.zip
grep 'Abituria-v0.9.3-osx-x64.zip' SHA256SUMS.txt
gh attestation verify Abituria-v0.9.3-osx-x64.zip --repo haribo841/Abituria
```

## Windows

1. Zweryfikuj pobrany plik `Abituria-v0.9.3-win-x64.exe`.
2. Przenieś go do wybranego katalogu, na przykład `%LOCALAPPDATA%\Programs\Abituria-0.9.3`.
3. Uruchom `Abituria-v0.9.3-win-x64.exe`. Rozpakowywanie nie jest potrzebne.
...
$app = Join-Path $env:LOCALAPPDATA "Programs\Abituria-0.9.3\Abituria-v0.9.3-win-x64.exe"
try {
...
3. Rozpakuj paczkę do nowego katalogu:
   ...
   ```bash
   mkdir -p "$HOME/.local/opt/abituria-0.9.3"
   tar -xzf Abituria-v0.9.3-linux-x64.tar.gz \
     -C "$HOME/.local/opt/abituria-0.9.3"
   cd "$HOME/.local/opt/abituria-0.9.3/Abituria-v0.9.3-linux-x64"
   chmod u+x ./Abituria
   ./Abituria
   ```
...
2. Rozpakuj ZIP. Archiwum utworzy katalog `Abituria-v0.9.3-osx-x64`, wewnątrz którego znajduje się kompletne `Abituria.app`.
3. Przenieś aplikację `Abituria-v0.9.3-osx-x64/Abituria.app` do katalogu `/Applications` albo uruchamiaj ją z rozpakowanego katalogu użytkownika.
4. Otwórz aplikację.
...
# app="$PWD/Abituria-v0.9.3-osx-x64/Abituria.app"
...
3. Pobierz nowy artefakt, jego sumy i zweryfikuj attestation.
4. Na Windows zapisz nowy plik EXE w nowym katalogu. Na Ubuntu i macOS rozpakuj nową wersję do nowego katalogu zamiast mieszać pliki z dwóch wersji.
...
1. Zamknij Abiturię.
2. Usuń plik EXE Windows, rozpakowany katalog programu albo `Abituria.app`.
