...
Data ostatniego lokalnego wykonania: 4 września 2026 r., Windows 11 x64, .NET SDK `10.0.302`, Python `3.13.1`, konfiguracja `Release`.

Pełny przebieg `dotnet test Abituria.sln --configuration Release --no-build --no-restore` z raportem OpenCover zakończył się wynikiem `655/655 PASS`. OpenCover wykazał `95,89%` pokrycia linii (`4825/5032`) i `85,03%` pokrycia gałęzi (`1851/2177`) kodu C#. Dziewięć testów Python generatora PDF i importera przykładów CKE przeszło, a `coverage.py` wykazał `98,10%` linii (`309/315`) i `92,05%` gałęzi (`81/88`). Wspólna bramka zakończyła się wynikiem `92,83%` pokrycia łącznego i `85,30%` gałęzi, powyżej wymaganych progów `90%` i `85%`.
...
| Treści | `ContentInventoryTests`, `ContentSeparationTests`, `Issue35MathChaptersRegressionTests`, `Formula2023ContentTests`, `MathCourse2023ContentTests`, `OfficialCourseExampleContentTests`, `Formula2015ArchiveContentTests`, `Matura2019ContentTests` do `Matura2026ExtendedContentTests`, `DiagramCatalogTests`, `LegacyImageArchiveTests` | kompletność tablic i kursu Formuły 2023, autorski kontrakt `4/13/73/46/238/357`, dodatkowe przykłady CKE `66/31/97`, 46 aktywnych arkuszy i 1 281 jednostek postępu, przypięte źródła oraz SHA-256 Formuły 2015 od 2015 r., 249 aktywnych diagramów, zgodność archiwum 75 obrazów, format JSON i renderowanie treści |
...
Publiczne wydanie `0.9.3` jest osobnym procesem. Zwykły walidator proweniencji musi przechodzić na bieżącym worktree. Brama `-RequireReleaseEligible` jest obecnie oczekiwanie blokowana przez nowe transkrypcje i 54 figury CKE z 2017, 2016 oraz Formuły 2015 z 2015 i lat równoległych 2023-2026, których nie obejmuje deklaracja z 1 września 2026 r. Dopiero po odrębnym rozszerzeniu praw ma ona przechodzić przed kolejnym wydaniem. Zmiana dowolnego źródła, zakresu redystrybucji lub implementacji diagramów wymaga ponownego uruchomienia obu bramek.
