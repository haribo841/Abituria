...
| F-08 | System pokazuje 46 arkuszy i 1 281 jednostek postępu: pełny katalog Formuły 2015 od 2015 r., równoległe arkusze Formuły 2015 z lat 2023-2026 oraz arkusze Formuły 2023 z lat 2023-2026. Każdy aktywny arkusz ma przypięte źródło, rozwiązanie i kryteria. Obie poprawki PP z sierpnia 2026 nie są aktywne do czasu publikacji zasad oceniania. | Wysoki | Zaimplementowane technicznie, publiczna redystrybucja materiałów CKE z grup `blocked` wymaga rozszerzenia deklaracji praw | `Content/exams.json`, `Content/exam-2015-*.json`, `Content/exam-20??-f2015-*.json`, `Formula2015ArchiveContentTests` |
| F-09 | System umożliwia wybór arkusza, losowanie tylko z wybranego arkusza oraz agregację czterdziestu sześciu arkuszy według 17 tematów. | Wysoki | Zaimplementowane | `MaturaView`, `TaskTopicsView`, `ContentRepository`, `Matura2016ContentTests`, `Matura2026UiTests` |
...
| F-24 | Profil pokazuje osobno postęp czterdziestu sześciu arkuszy, w tym 2016 PP `x/34`, PR `x/16` i poprawkowej PP `x/34`, 2017 PP `x/34`, PR `x/15` i poprawkowej PP `x/34`, oraz zachowane liczniki matur 2018-2026 i poprawki 2021. Pokazuje także podstawy `x/219` i część rozszerzoną `x/138`, bez zmiany schematu SQLite. | Wysoki | Zaimplementowane | `ProfileView`, `AccountService`, `Matura2016ContentTests`, `Matura2026UiTests`, `MathCourse2023ContentTests` |
...
| NF-16 | Strona „Opcje” udostępnia ustawienie systemowe oraz motyw jasny, ciemny i wysokiego kontrastu, bez zajmowania miejsca na pasku tytułu i bez wymuszania wariantu jasnego. | Wysoki | Zaimplementowane | `OptionsView`, `AppThemeManager`, `AppStyles.axaml`, testy renderowania motywów |
...
| NF-22 | Aktywne treści używają dokładnie 249 skalowalnych diagramów z opisami alternatywnymi i nie ładują rastrów; `img/icon.ico` pozostaje jedynym statycznym wyjątkiem jako ikona aplikacji. | Wysoki | Zaimplementowane | `Content/diagrams.json`, `DiagramView`, `DiagramCatalogTests`, `LegacyImageArchiveTests` |
...
| Zadania maturalne | Indeks arkuszy, lista zadań, tematy, odpowiedzi proste i złożone, rozwiązania, punktacja oraz postęp | `ExamViews.cs`, `Content/exams.json`, `Content/exam-2015-*.json`, `Content/exam-20??-f2015-*.json`, `Content/exam-2024-main-extended.json`, `Content/exam-2025-main-*.json`, `Content/exam-2026-main-*.json`, `Content/exam-2021-correction.json` |
...
7. Inwentarz treści potwierdza 18 tablic, kompletne odwzorowanie 17 sekcji CKE, 91 kątów w tabeli trygonometrycznej, 4 grupy i 13 obszarów kursu, autorski kontrakt `119/238/357`, dodatkowy kontrakt CKE `66/31/97`, 17 tematów oraz 46 aktywnych arkuszy i 1 281 jednostek postępu Formuły 2015-2026 i Formuły 2023-2026.
8. Wszystkie 249 identyfikatorów diagramów wskazanych przez treści istnieją, są używane i mają niepuste opisy alternatywne; 201 figur matur 2015-2026 ma stronę i źródło.
...
34. Kryteria Issue #6 wymagają własnego paska bez dekoracji systemowych, historycznej kolejności `🍓` zamknij, `🍋` pełny ekran lub przywróć, `🍏` minimalizuj po lewej, wyśrodkowanej marki `🍀 Abituria`, fontu Mulish oraz tooltipów i nazw automatyzacji wszystkich kontrolek. Wybór motywu został następnie przeniesiony z paska do strony „Opcje”.
...
38. Matura maj 2023 PR wymaga przypiętego arkusza i zasad oceniania CKE z SHA-256, kontraktu `13/14/50`, stabilnych identyfikatorów `mm23-r0-*`, czterech diagramów wektorowych, agregacji w 17 tematach, osobnego postępu oraz statusu proweniencji `approved` na podstawie osobistego rozszerzenia deklaracji praw z 12 sierpnia 2026 r.
39. Pełne archiwum Formuły 2015 wymaga arkuszy od 2015 r., równoległych arkuszy F2015 z lat 2023-2026, przypiętych źródeł i SHA-256, stabilnych identyfikatorów bez zmiany postępu SQLite, 46 aktywnych arkuszy oraz 1 281 jednostek postępu. Obie poprawki z sierpnia 2026 r. pozostają nieaktywne do publikacji zasad oceniania, a nowe grupy zachowują status `blocked` do czasu indywidualnej zgody na redystrybucję.
...
Techniczny inwentarz licencji i proweniencji obejmuje matury CKE 2015-2026, ich zasady oceniania oraz autorskie implementacje wektorowe Avalonia. Rozszerzenia deklaracji właściciela z 3, 5, 10 i 12 sierpnia oraz 1 września 2026 r. obejmują zatwierdzone arkusze 2018-2026, 97 przykładów z informatorów CKE i wcześniej zatwierdzone diagramy. Grupy `cke-2023-main-extended-exam`, `cke-2024-main-basic-exam` oraz `cke-2024-main-extended-exam` zachowują status `approved`. Dwadzieścia grup arkuszy Formuły 2015 z 2015-2017 oraz lat równoległych 2023-2026, a także wspólny katalog diagramów, mają status `blocked`, dlatego zwykła walidacja proweniencji przechodzi, ale `Test-ContentProvenance.ps1 -RequireReleaseEligible` celowo nie przechodzi na bieżącym worktree. Rozszerzenie deklaracji właściciela z 1 września 2026 r. zatwierdzało dziewiętnaście wcześniej zablokowanych grup, w tym matury 2018-2020, matury główne 2021 i matury główne oraz poprawkową 2022 Formuły 2015, wraz z 195 ówczesnymi diagramami.

## Status issue #36

Repozytorium zawiera kompletny proces wydania wersji `0.9.3`: jedno źródło wersji, .NET 10 LTS, lockfile, audyt, paczki portable dla trzech systemów, diagnostyczny smoke test, ekran "O programie", dokumentację, sumy, SBOM i atestacje. Opublikowany prerelease jest śledzony pod adresem `https://github.com/haribo841/Abituria/releases/tag/v0.9.3`. Bieżący manifest ma `releaseEligible=false`, ponieważ lokalne materiały CKE Formuły 2015 z 2015-2017 oraz lat równoległych 2023-2026 nie mają jeszcze osobnego rozszerzenia deklaracji praw.

## Status Issue #9

Bieżące [haribo841/Abituria#9](https://github.com/haribo841/Abituria/issues/9) wymaga utrzymania kompletnej analizy biznesowej obejmującej cele, użytkowników, model udostępniania, zakres, harmonogram, licencję, metodykę wymagań i architekturę. Aktywny dokument [BUSINESS_ANALYSIS.md](BUSINESS_ANALYSIS.md) zawiera weryfikowalne wymagania, testy i bramę pochodzenia zasobów. Historyczny kontekst pozostaje wyłącznie pod pełnym adresem [Projekt-Inzynierski-AK-AS-FD/Projekt-Inzynierski#38](https://github.com/Projekt-Inzynierski-AK-AS-FD/Projekt-Inzynierski/issues/38), bez zmian w starym repozytorium. Bieżące `releaseEligible=false` odzwierciedla blokadę nowych paczkowanych transkrypcji Formuły 2015 z 2015-2017 oraz lat równoległych 2023-2026 do czasu rozszerzenia deklaracji właściciela.
