...
Źródło uzupełnienia: issue #34 "Uzupełnić: Dokument wymagań projektowych", issue #35 "Uzupełnić: Treść działów matematyki", issue #36 "Implementacja i wdrożenie" oraz aktualny stan repozytorium `haribo841/Abituria`.
...
| NF-13 | Paczki portable nie zawierają PDB, sekretów ani snapshotów i otrzymują sumy SHA-256, SBOM oraz atestację pochodzenia. | Wysoki | Zaimplementowane w automatyzacji | workflow wydania, skrypty `tools/release` |
...
17. Każda paczka zawiera `LICENSE`, skróconą instrukcję i `THIRD-PARTY-NOTICES.md`, ale nie zawiera PDB, sekretów ani starych snapshotów.
18. Sumy w `SHA256SUMS.txt` zgadzają się z paczkami, każda paczka ma osobny SBOM SPDX, a artefakty mają atestację pochodzenia.
...
| Treści edukacyjne | F-06 - F-13, F-19, NF-04, NF-08 | `Content/*.json`, `ContentRepository`, `RichContentView`, `ExamViews` | `ContentInventoryTests`, `ContentSeparationTests`, `Discussion10VisualRegressionTests`, `Issue35MathChaptersRegressionTests` |
...
| Wydanie i łańcuch dostaw | F-20 - F-21, NF-11 - NF-13 | `Directory.Build.props`, `global.json`, lockfile, `tools/release`, workflow wydania | `ReleaseRuntimeTests`, `ContentProvenanceTests`, `docs/RELEASE_PROCESS.md` |
...
Repozytorium zawiera implementację procesu przygotowania wersji `0.9.0-beta.1`: jedno źródło wersji, .NET 10 LTS, lockfile, audyt, paczki portable dla trzech systemów, diagnostyczny smoke test, ekran "O programie", dokumentację, sumy, SBOM i atestacje. Zakończenie techniczne nie jest równoznaczne z publicznym wydaniem. Publikacja prerelease pozostaje zabroniona, dopóki `Content/provenance.json` wskazuje nierozstrzygnięte prawa do materiałów CKE albo odziedziczonych grafik. Issue #36 można zamknąć dopiero po publicznym prerelease, działającej stronie Pages i komentarzu z linkami do workflow oraz checklisty.
