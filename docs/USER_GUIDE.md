...
Podręcznik dotyczy Abiturii `0.9.3`. Aplikacja działa lokalnie i nie wymaga połączenia z internetem do nauki, obliczeń ani zapisu postępu.
...
Strona „Profil” pokazuje osobno postęp każdego z 46 arkuszy, na przykład `Matura maj 2026 PP: x/37`, `Matura maj 2026 PP (F2015): x/34`, `Matura maj 2015 PR (F2015): x/16`, a także podstawę `x/219` oraz część rozszerzoną `x/138`. Postęp jest lokalny i oddzielny dla każdego profilu. Nowe liczniki używają istniejącego zapisu identyfikatorów ukończonych zadań, więc aktualizacja nie zmienia schematu bazy i nie usuwa wcześniejszego postępu `mp21-*`.
