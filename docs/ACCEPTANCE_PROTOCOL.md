...
| ograniczenia i status prawny zasobów | `KNOWN_LIMITATIONS.md`, `CONTENT_PROVENANCE.md`, `ASSET_RIGHTS_DECLARATION.md` | PASS dla historycznego wydania `0.9.3`; manifest jego commita miał `releaseEligible=true` po rozszerzeniu deklaracji właściciela z 1 września 2026 r. |
...
Właściciel poświadczył posiadanie praw do zakresu zasobów ocenianego 19 lipca 2026 r., a 3, 5, 10 i 12 sierpnia oraz 1 września 2026 r. rozszerzył deklarację na wszystkie paczkowane arkusze CKE 2018-2026, zasady oceniania, przykłady z informatorów CKE oraz 195 autorskich implementacji wektorowych Avalonia. Historyczne przekazanie prowadzącemu zostało uzgodnione i zaakceptowane, dlatego spełnia formalny wariant przekazania dla Issue #43. Dokładny commit wydania `0.9.3` miał `releaseEligible=true`; bieżący worktree zawiera dodatkowe, zablokowane zasoby 2017, 2016 oraz Formuły 2015 z 2015 i lat równoległych 2023-2026, więc ma `releaseEligible=false`. Oba warianty walidatora muszą przejść przed każdym nowym wydaniem.
