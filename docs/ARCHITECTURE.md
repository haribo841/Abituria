...
- `exam-2015-*.json` - pierwsza sesja Formuły 2015: PP, PR i poprawkowa PP, łącznie 84 jednostki postępu,
- `exam-2023-f2015-*.json`, `exam-2024-f2015-*.json`, `exam-2025-f2015-*.json` - równoległe sesje Formuły 2015: PP, PR i poprawkowa PP,
- `exam-2026-f2015-*.json` - równoległa sesja Formuły 2015: PP i PR; poprawka PP oczekuje na publikację zasad oceniania,
...
- `diagrams.json` - 249 deterministycznych diagramów wektorowych, w tym 201 figur matur 2015-2026 ze stroną źródłową,
...
Kod produkcyjny odpowiada za wczytanie i wyświetlenie treści, a nie za przechowywanie długich materiałów edukacyjnych. `ContentRepository.Exams` zachowuje kolejność aktywnego indeksu, `GetExam(id)` wybiera arkusz, a `GetTopicExercises(topicId)` agreguje zadania z czterdziestu sześciu arkuszy bez zmiany identyfikatorów `mp21-*`. `ExamCatalogValidator` sprawdza indeks, metadane, globalną unikalność identyfikatorów, kolejność, tematy i umowy odpowiedzi.
...
- `Matura2016ContentTests` do `Matura2026ExtendedContentTests`, `Matura2026UiTests`, `Formula2015ArchiveContentTests` i `CompoundAnswerEvaluatorTests` - kontrakty matur oraz źródeł, pełna oś Formuły 2015 od 2015 r., równoległe arkusze F2015 z lat 2023-2026, 46 arkuszy oraz `1 281` jednostek postępu, agregacja tematów, odpowiedzi złożone i oddzielny postęp,
