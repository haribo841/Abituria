...
class ImportCkeInformerExamplesTests(unittest.TestCase):
    def test_text_helpers_strip_headers_normalize_pages_and_map_requirements(self) -> None:
        ...
        self.assertEqual("Treść", cleaned)
        ...
        self.assertEqual(["III.E.5", "I.B.1", "I.E.1"], requirements)
        self.assertEqual(
            ["XIII.B.1"],
    ...
    def test_points_pages_hash_and_argument_helpers_cover_boundaries(self) -> None:
        self.assertEqual(3, PDF_MODULE._maximum_points("Zadanie 4. (0–3)", 4))
        self.assertEqual(
            5,
        ...
        self.assertEqual([12, 13], PDF_MODULE._source_pages(chunks, 20, 45))

        with tempfile.TemporaryDirectory() as directory:
            path = Path(directory) / "source.bin"
            path.write_bytes(b"Abituria")
            self.assertEqual(hashlib.sha256(b"Abituria").hexdigest().upper(), PDF_MODULE._sha256(path))
        args = PDF_MODULE._parse_args(
            ["--basic-pdf", "basic.pdf", "--extended-pdf", "extended.pdf"]
        )
        self.assertEqual(Path("Content/official-course-examples.json"), args.output)
    ...
    def test_extract_examples_builds_complete_record_and_rejects_bad_inputs(self) -> None:
        ...
        self.assertEqual("cke-basic-guide-task-01", example["id"])
        self.assertEqual([1, 2], example["sourcePages"])
        self.assertEqual(["I.B.1"], example["requirementIds"])
        self.assertEqual(2, example["maximumPoints"])
    ...
    def test_build_write_and_main_keep_the_catalog_deterministic(self) -> None:
        ...
        self.assertEqual([basic_example, extended_example], catalog["examples"])
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(catalog, json.loads(data.decode("utf-8")))
    ...
    def test_visual_reference_metadata_is_complete(self) -> None:
        self.assertEqual(53, sum(len(items) for items in PDF_MODULE.VISUAL_DESCRIPTIONS.values()))
        self.assertEqual(39, len(PDF_MODULE.VISUAL_DESCRIPTIONS))
